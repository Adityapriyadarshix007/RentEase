...
const AdminAnalytics = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/api/admin/export?type=${exportType}const handleExport = async () => {startDate=${dateRange.startDate}const handleExport = async () => {endDate=${dateRange.endDate}`;
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/api/admin/export?type=${exportType}&startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
